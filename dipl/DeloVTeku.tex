...
\begin{minipage}{0.5\textwidth}
	...
	\node[right] at (0, 0) {$x_0$};
	\node[right] at (0.25, 1) {$s_f$};
	\node[right] at (0, -1) {$s_g$};
	\end{tikzpicture}
...
Tako smo definirali presečno število komponente, a preden nadaljujemo, opazimo še, da lahko v zadnjem primeru, ko je $K \approx J$ in $\ind[K]{f}{g} \neq 0$ presečno število določimo kar z obravnavo usmerjenosti smernih vektorjev v enem oglišču $K$. Označimo kot zgoraj vektor $\overrightarrow{a}$, a zdaj nadaljujmo z vrtenjem v pozitivni smeri okrog $f(a)$, dokler se vektor spet ne prilega $f$ ali $g$. Dobljeni vektor, ki mu smer kot prej določimo glede na krivuljo, ki se ji prilega, označimo s $\overrightarrow{c}$. Ta primer prikazuje slika \ref{fig:presecno_daljica_3}. Velja, da je $\ind[K]{f}{g}$ enako $1$, če je $\overline{a} \subset f(I)$ in sta usmerjenosti $\overrightarrow{a}, \overrightarrow{c}$ glede na $K$ enaki ali $\overline{a} \subset g(I)$ in sta usmerjenosti $\overrightarrow{a}, \overrightarrow{c}$ glede na $K$ različni.  Sicer je presečno število enako $-1$. 
...
Naj bosta $f$ in $\widetilde{f}$ kosoma linearni in naj se razlikujeta le v poti med $f(a)$ in $f(b)$, kjer sta $a,\ b \in (0, 1),\ a<b$ taki točki, da je $f$ med njima linearna, $\widetilde{f}$ pa med tema točkama sestavljata le dve daljici. Če leži trikotnik med omenjenimi daljicami v celoti znotraj $X \setminus \{g(0), g(1)\},$ je $\ind{f}{g} = \ind{\widetilde{f}}{g}$. 
\end{lema}
...
\proof
Označimo omenjeni trikotnik s $T$. Dovolj je gledati preseke $f,\ \widetilde{f}$ in $g$ na območju T, saj se drugje $f$ in $\widetilde{f}$ ujemata. Ker je tudi $g$ kosoma linearna, si najprej pogledamo kako lahko usmerjena daljica $D$, ki seka $T$, vpliva na presečni števili. Kot prikazuje spodnja slika sta možnosti naslednji.
...
Da najdemo objekt, ki je homeomorfen dobljenemu trikotniku, ga moramo najprej prerezati vzdolž premice $y = 1 - x$. Sedaj lahko zlepimo prvotna kraka trikotnika in dobimo kvadrat, z nasprotno orientiranim parom stranic. Takšen kvadrat je homeomorfen Mobiusovem traku brez robnih točk, saj kvadrat ni vseboval točk drugega para stranic. Opisani postopek ponazarja zaporedje korakov na sliki \ref{fig:koraki_do_Mob}. Injektivnost preslikave $f$ je tako ekvivalentna vložitvi Mobiusovega traku v $\R^3$, kjer se vse robne točke traka preslikajo v Jordanovo krivuljo v ravnini $z=0$.
