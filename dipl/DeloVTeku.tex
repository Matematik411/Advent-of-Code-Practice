...
\povzetek{Ali mora vsaka slika v okolje figure? \\ Na slike se med tekstom eksplicitno sklicujem? \\ Ali lahko imam nekaj barv?}
...
Na začetku ponovimo in vpeljimo nekatere pojme, ki jih bomo uporabljali pri raziskovanju teme.

\subsection{Terminologija}

Tekom naloge bomo pogosto iskali točke, ki tvorijo oglišča enakostraničnega trikotnika. To lastnost točk $x,\ y$ in $z$ bomo na krajše zapisali kot $xyz \sim \triangle$. Nadalje bomo za točko $x \in X$ rekli da je \emph{ogliščna točka} danega objekta $X$, če obstajata taki točki $y,\ z \in X$, da $xyz \sim \triangle$.

Ker si bomo natančneje pogledali dogajanje v ravnini, ponovimo definicijo Jordanove krivulje.
\begin{definicija}
...
\end{definicija}
Glavni izrek glede Jordanovih krivulj je Jordanov izrek, ki nam pove, da Jordanova krivulja $J$ deli $\R^2 \setminus J$ na dve komponenti za povezanost. Natanko ena izmed teh komponent je omejena in tej komponenti bomo rekli kar \emph{notranjost Jordanove krivulje J}. V nalogi bomo tudi uporabljali pojem homotopnih preslikav, zato si oglejmo njihovo definicijo.
\begin{definicija}
...
\end{definicija}
Za homotopijo rečemo, da miruje na robu, če $\forall t \in [0, 1]$ velja $H(x, t) = f(x) = g(x)$ za vsako točko $x \in \partial X$ na robu množice $X$.
...
\end{izrek}
\begin{dokaz}
Naj bo $J$ poljubna Jordanova krivulja v ravnini in $S$ poljubna točka v notranjosti J. Naj bo $C$ krožnica z najmanjših polmerom s središčem v $S$, ki se dotika $J$. Označimo z $x$ točko v preseku $C \cap J$ ter naj bosta $y_0$ in $z_0$ taki točki na $C$, da $xy_0z_0 \sim \triangle$. Definiramo še spremenljivi točki $y$ in $z$ v ravnini, tako da za $x,\ y$ in $z$ vselej velja $xyz \sim \triangle$. Ko bomo torej premikali eno izmed $y$ ali $z$, se bo premikala tudi druga, tako da se bo ohranjal enakostranični trikotnik $xyz$. Za njuni začetni vrednosti definiramo $y_0$ in $z_0$ na $C$.
...
\end{tikzpicture}
\caption{Označene točke}
\end{figure}

Sedaj pri fiksnem $x$ začnimo povečevati polmer krožnice $C$. Tako se $y$ in $z$ oddaljujeta od $x$. Postopek ustavimo, ko $y$ ali $z$ pristaneta na $J$ in na tej točki označimo $y_1 = y$ ter $z_1 = z$. Brez škode za splošnost lahko rečemo $y \in J$. Za konec premaknimo $y$ zvezno do točke $y_2$, ki je točka na $J$, ki je od $x$ najbolj oddaljena.
...
Izrek nam zagotovi obstoj včrtanega enakostraničnega trikotnika na ravninski Jordanovi krivulji. Koliko takšnih trikotnikov pa lahko najdemo? Naslednja posledica, katere dokaz sledi iz dokaza izreka \ref{izr:zacetni}, nam poda karakterizacijo ogliščnih točk Jordanove krivulje.
...
\end{trditev}
\begin{dokaz}
Postopek dokaza je enak postopku dokaza izreka \ref{izr:zacetni}, zato ga še enkrat ponovimo in izpostavimo spremembe. Krožnico $C$ in točko $x$ definiramo kot prej, spremenljivi točki $y$ in $z$ pa tokrat skupaj z $x$ vselej tvorita oglišča trikotnika podobnega $T$ tako, da je največji notranji kot $T$ pri oglišču $x$. Njuna začetna položaja $y_0$ in $z_0$ naj ležita na $C$. Definirajmo še točki $x_3$ in $y_2$ kot točki na $J$, ki sta najbolj oddaljeni.

Zdaj izvedemo zvezna premikanja točk $y$ in $z$ kot v omenjenem dokazu in na koncu naredimo še naslednji premik. Pri fiksnem $y = y_2$ po krivulji $J$ premaknimo $x$ do $x_3$. Skupna pot točke $z$ je tako sestavljena iz treh zveznih lokov. Ker je $yz$ (ob koncu $y_2z_3$) najdaljša stranica trikotnika, po podobnem premisleku kot prej sledi, da mora $z_3$ ležati na $J$ ali v neomejeni komponenti $\R^2 \setminus J$. Željeni rezultat sledi.
\end{dokaz}

V tem razdelku smo videli konstruktivne dokaze za obstoj enakostraničnih trikotnikov na Jordanovih krivuljah. Da bi se tega lotili bolj sistematično, potrebujemo nekaj novih ravninskih pojmov in objektov.


%--------------------------------------------------------------
\section{Presečno število}\label{razd:presecno}
Glavni nov pojem, ki ga potrebujemo za nadaljne delo je pojem presečnega števila. Za dani krivulji v ravnini, bi radi načrtno prešteli njune točke preseka. S takšnih znanjem bomo kasneje lahko natančno povedali, kje bodo obstajali enakostranični trikotniki.

Naj bo $I = [0, 1] \subset \R$ in naj bosta $f,\ g \colon I \to X \subset \R^2,$ taki ravninski krivulji, da zadoščata pogoju
\begin{equation}\label{eq:razl_rob}
\{f(0), f(1)\} \cap g(I) = \emptyset \quad \text{in} \quad \{g(0), g(1)\} \cap f(I) = \emptyset.
\end{equation}
Pojem presečnega števila lahko dobro definiramo le za pare krivulj, ki ustrezajo \eqref{eq:razl_rob}, zato se omejimo na takšne.

Naj bosta za začetek $f$ in $g$ kosoma linearni ravninski krivulji, ki ustrezata \eqref{eq:razl_rob}. Ker se poljubni daljici sekata v kvečjemu eni komponenti, velja da je $f(I) \cap g(I)$ zaprta množica, ki vsebuje kvečjemu končno mnogo komponent za povezanost. Te komponente so lahko točke ali kosoma linearne krivulje. Presečno število krivulj bomo definirali kot vsoto presečnih števil komponent njunega preseka, zato si najprej oglejmo, kako definiramo presečno število posamezne komponente.

Naj bo $K \subset f(I) \cap g(I)$ komponenta za povezanost, za katero bomo definirali presečno število komponente in ga označili z $\ind[K]{f}{g}$. Najprej denimo, da je $K$ le točka, torej $K = \{x\}.$ Za primer ko je $x$ točka nelinearnosti $f$ ali $g$ in se krivulji v $x$ ne prečkata, definiramo $\ind[K]{f}{g} = 0.$ Sicer označimo smerna vektorja krivulj $f$ in $g$ v točki $x$ kot $s_f$ in $s_g$. Kot presečno število komponente pa definiramo
\begin{equation*}
\ind[K]{f}{g} = \begin{cases}
1; &s_f,\ s_g\ \text{tvorita pozitivno orientirano bazo,}\\
-1; &\text{sicer}.
\end{cases}
\end{equation*}
...
Druga možnost je, da velja $K \approx J$ za nek interval $J$. Tedaj velja $K = f([a, b])$, za $a,\ b \in (0, 1),\ a < b.$ Tedaj obstaja tak $\varepsilon > 0$, da sta preseka zaprtih krogel z unijo krivulj, $\overline{K(f(a), \varepsilon)} \cap (f(I) \cup g(I))$ in $\overline{K(f(b), \varepsilon)} \cap (f(I) \cup g(I))$ linearni triodi s stičišči krakov v $f(a)$ oz. $f(b).$ Linearna trioda je trioda, ki ima za krake daljice. Za določitev presečnega števila najprej preverimo, ali je prišlo do prečkanja krivulj. Zato zavrtimo daljico $\overline{f(a)f(a+\varepsilon)}$ v pozitivni smeri okrog $f(a)$ in daljico $\overline{f(b)f(b-\varepsilon)}$ v negativni smeri okrog $f(b)$, dokler se ne prilegata $f$ ali $g$. Če se po koncu vrtenja obe zavrteni daljici prilegata sliki iste funkcije, definiramo $\ind[K]{f}{g} = 0.$ Sicer nas zanima še relativna usmerjenost teh odsekov. Označimo končni legi zgornjih daljic z vektorjema $\overrightarrow{a}$ in $\overrightarrow{b}$, kjer za smer vzamemo smer krivulje $f$ oziroma $g$ na tem intervalu. Zanima nas ali kažeta vektorja v komponento $K$ ali iz nje. Presečno število komponente definiramo kot $1$, če velja $\overline{a} \subset f(I)$ in sta usmerjenosti $\overrightarrow{a}, \overrightarrow{b}$ glede na $K$ različni ali pa velja $\overline{a} \subset g(I)$ in sta usmerjenosti $\overrightarrow{a}, \overrightarrow{b}$ glede na $K$ enaki. Sicer je presečno število komponente enako $-1$. 
...
Tako smo definirali presečno število komponente, a preden nadaljujemo, opazimo še, da lahko v zadnjem primeru, ko je $K \approx J$ in $\ind[K]{f}{g} \neq 0$ presečno število določimo kar z obravnavo usmerjenosti smernih vektorjev v enem oglišču $K$. Označimo kot zgoraj vektor $\overrightarrow{a}$, a zdaj nadaljujmo z vrtenjem v pozitivni smeri okrog $f(a)$, dokler se vektor spet ne prilega $f$ ali $g$. Dobljeni vektor, ki mu smer kot prej določimo glede na krivuljo, ki se ji prilega, označimo s $\overrightarrow{c}$. Velja, da je $\ind[K]{f}{g}$ enako $1$, če je $\overline{a} \subset f(I)$ in sta usmerjenosti $\overrightarrow{a}, \overrightarrow{c}$ glede na $K$ enaki ali $\overline{a} \subset g(I)$ in sta usmerjenosti $\overrightarrow{a}, \overrightarrow{c}$ glede na $K$ različni.  Sicer je presečno število enako $-1$. 
...
Kot presečno število kosoma linearnih krivulj lahko zdaj definiramo 
\[
...
Naj bosta $f$ in $\widetilde{f}$ kosoma linearni in naj se razlikujeta le v poti med $f(a)$ in $f(b)$, kjer sta $a,\ b \in (0, 1),\ a<b$ taki točki, da je $f$ med njima linearna, $\widetilde{f}$ pa med tema točkama sestavljata le dve daljici. 

Če leži trikotnik med omenjenimi daljicami v celoti znotraj $X \setminus \{g(0), g(1)\},$ je $\ind{f}{g} = \ind{\widetilde{f}}{g}$. 
\end{lema}
...
\begin{dokaz}
Označimo omenjeni trikotnik s $T$. Dovolj je gledati preseke $f,\ \widetilde{f}$ in $g$ na območju T, saj se drugje $f$ in $\widetilde{f}$ ujemata. Ker je tudi $g$ kosoma linearna, si najprej pogledamo kako lahko usmerjena daljica $D$, ki seka $T$, vpliva na presečni števili.

Lahko sta začetna in končna točka $D$ zunaj $T$. Tedaj $D$ bodisi seka dvakrat $\widetilde{f}$ in nikoli $f$ bodisi vsako po enkrat. A ker daljica $D$ ohranja svojo smer, prav tako pa sta $f$ in $\widetilde{f}$ enako usmerjeni, se v prvem primeru spremembi presečnega števila $\widetilde{f}$ in $g$ izničita, v drugem pa sta spremembi obeh presečnih števil enaki ($+1$ ali $-1$).

Druga možnost je, da daljica $D$ začne zunaj $T$, konča pa znotraj. Tedaj mora po predpostavki obstajati neka druga daljica, skozi katero $g$ zapusti $T$, in vpliv tega primera na presečni števili je enak kot prej. Spremembi obeh presečnih števil sta zopet enaki.

\begin{figure}[h!!] % a tu res potrebujem BEGIN FIGURE?? -- vrže mi na napačno mesto
\centering
\begin{tikzpicture}
...
\end{tikzpicture}
\caption{Ločitev primerov pri dokazu leme \ref{le:trikotnik}}
\end{figure}


Ostane obravnavati primer, ko se $g$ ujema s katero krivuljo v kosoma linearnem odseku. Če je presečno število komponente ujemanja z eno izmed $f, \widetilde{f}$ enako $0$, potem je tako tudi z drugo, ali pa se $g$ nadaljuje v notranjost $T$, kjer nato izhod iz $T$ izniči spremembo presečnega števila v $f(a)$ ali $f(b)$. V primeru neničelnih sprememb presečnih števil so te spet enake, saj se $g$ oddaljuje v isto smer glede na $f$ in $\widetilde{f}$.

Ti premisleki hitro sledijo iz definicije presečnega števila komponente in so jasni, ko si narišemo skico možnih presekov kosoma linearnih $f,\ \widetilde{f}$ in $g$.
\end{dokaz}
...
Za lažji zapis in računanje, si v tem razdelku ravnino predstavljamo kot kompleksno ravnino. Naj bosta $A \subset \C$ in $x \in \C$ poljubna. Definiramo $A_x$ kot rotacijo množice $A$ okrog $x$, natančneje $A_x = \{ y \in \C \mid \exists z \in A: y = k(z-x) + x\}$, kjer je $k = e^{i\pi / 3}$. Če je $A$ orientiran lok, $A_x$ orientiramo tako, da je začetna točka loka $A_x$ slika začetne točke loka $A$ pri rotaciji.
...
\item Če ne obstajata taka $y,\ z \in A$, da $xyz \sim \triangle$, sta $A$ in $A_x$ disjunktna in zato je $ind(A, A_x) = 0$.
...
$(1)$ Zveznost je lokalna lastnost, zato nas zanimajo le možne spremembe pri majhnih spremembah točke $x$. Pokažimo, da se pri takih dogodkih presečno število ne bo spremenilo. Edini položaj krivulj, kjer lahko pri majhnih spremembah točke $x$, kakšna točka preseka izgine ali nastane je, da se krivulji sekata v točki $K$ nelinearnosti ene izmed krivulj ter se v tej točki ne prečkata. V tem primeru je $\ind[K]{A}{A_x}=0$. Po tresljaju točke $x$ se lahko zgodi, da v okolici $K$ ne bo več nobene točke preseka, kar pomeni, da se presečno število ni spremenilo. Lahko pa v okolici $K$ nastane med krivuljama trikotnik. Tako obstajata zdaj dve točki preseka, a ker je relativna smer krivulje, ki predstavlja dve stranici trikotnika, na drugo krivuljo v obeh presečiščih različna, imata presečni števili obeh presečišč različna predznaka. Ko ju seštejemo se torej izničita. Tako smo pokazali, da je pri dovolj majhnih tresljajih točke $x$ presečno število konstantno. Zato je število $\ind{A}{A_x}$ kot funkcija $x$ zvezno.
...
$(3)$ To točko dokažemo z dokazom obratne implikacije. Denimo, da $A$ in $A_x$ nista disjunktna. Označimo $\alpha \in A \cap A_x$ poljubno točko iz preseka. Tedaj je $\alpha \in A$ in obstaja $\gamma \in A$, da velja $\gamma_x = \alpha$. S temi oznakami velja $\alpha x \gamma \sim \triangle$.
...
Predpostavimo, da za triodo $T$ izrek \ref{izr:glavni} ne drži. Tedaj obstaja trioda $T'$ z istimi končnimi točkami $e_1,\ e_2$ in $e_3$, ki vsebuje daljico $S$, za katero velja, da je eno izmed njenih krajišč točka $e_1$. Tedaj tudi za triodo $T'$ ne obstaja enakostraničen trikotnik, ki bi imel za eno izmed oglišč točko $e_1,\ e_2$ ali $e_3$.
\end{lema}
\begin{dokaz}
Zopet si ravnino, ki vsebuje $T$, predstavljajmo kot kompleksno ravnino $\C$. Tako velja $T \times T \subset \C \times \C = \C^2$. Za $j \in \{1, 2, 3\}$ definiram $E_j$ unije parov kompleksnih ravnin v $\C^2$ kot
\[
...
\draw[loosely dotted] (-5, -1) circle (2.693);
...
\node at (0.75, -1.5) {$L_3$};
\end{tikzpicture}
\caption{Trioda $T'$ v dokazu izreka \ref{izr:glavni}}
\end{figure}
...
Poleg trikotnikov lahko na Jordanovih krivuljah iščemo tudi drugačne objekte. Oglejmo si kakšne do kakšnih sklepov lahko pridemo ob iskanju šritikotnikov na Jordanovih krivuljah v ravnini. Na tem področju so nekatera vprašanja še danes odprta. 

Ne ve se še, ali lahko na poljubni Jordanovi krivulji najdemo oglišča kvadrata. To vprašanje je že leta 1911 postavil nemški matematik Otto Toeplitz, a še do danes ostaja nerešeno v povsem splošni obliki. Obstajajo rezultati, do katerih lahko pridemo ob dodatnih omejitvah glede krivulje na kateri iščemo kvadrat. Eden izmed najbolj znanih takšnih rezultatov je naslednji izrek.

\begin{izrek}[Stromquistov izrek]
Če je Jordanova krivulja $J$ ``dovolj lepa'', vsebuje oglišča kvadrata.
\end{izrek}

Izraz ``dovolj lepa'' krivulja $J$ pomeni, da za vsako točko $P$ na $J$ obstaja koordinatni sistem ravnine, v katerem lahko okolico točke $P$ na $J$ dobimo kot graf $y = f(x)$ za zvezno funkcijo $f$. Idejno si lahko rezultat tega izreka predstavljamo kot trditev, da na vsaki Jordanovi krivulji, ki smo jo uspeli s pisalom narisati na papir, obstajajo oglišča kvadrata. Dokaza izreka si ne bomo ogledali. K iskanju kvadratov na Jordanovih krivuljah lahko pristopimo tudi s pomočjo simetrij, ki nam sicer omejijo možne krivulje, a hkrati zmanjšajo težavnost iskanja kvadratov samih. Oglejmo si naslednji rezultat na to temo.
...
\end{tikzpicture}
\caption{Skica dokaza trditve \ref{trd:kvadrat}}
\end{figure}


Pokazati ostane, da točka presečišča vselej obstaja. Označimo s $P_b$ in $P_d$ točki na $J$, ki sta najmanj in najbolj oddaljeni od izhodišča. Ker rotacija $f$ ohranja razdaljo točk do izhodišča leži $f(P_b)$ v notranjosti $J$ ali na $J$, točka $f(P_d)$ pa v neomejeni komponenti $\R^2 \setminus J$ ali na $J$. Če katera izmed omenjenih točk leži na $J$, smo našli točko presečišča. V nasprotnem primeru mora slika $f(J)$ povezovati točki na različnih bregovih $J$ in jo tako, zaradi zveznosti $f$, zopet nekje seka.
\end{dokaz}
...
A nas zanima množica neurejenih parov točk na $J$, zato moramo enotski kvadrat še spremeniti. Zaradi neurejenosti, sta sedaj para $(P, Q)$ in $(Q, P)$ enaka za poljubna $P, Q \in J$. Zato v enotskem kvadratu enačimo vse pare oblike $(a, b)$ in $(b, a)$. Enotski kvadrat smo tako prepognili vzdolž premice $y = x$ in dobili enakokrat trikotnik z enačenima krakoma. Ker nas zanimajo pari različnih točk, točk na premici $y = x$ ne vključimo v trikotnik, saj predstavljajo posamezne točke na $J$. 
...
Da najdemo objekt, ki je homeomorfen dobljenemu trikotniku, ga moramo najprej prerezati vzdolž premice $y = 1 - x$. Sedaj lahko zlepimo prvotna kraka trikotnika in dobimo kvadrat, z nasprotno orientiranim parom stranic. Takšen kvadrat je homeomorfen Mobiusovem traku brez robnih točk, saj kvadrat ni vseboval točk drugega para stranic. Injektivnost preslikave $f$ je tako ekvivalentna vložitvi Mobiusovega traku v $\R^3$, kjer se vse robne točke traka preslikajo v Jordanovo krivuljo v ravnini $z=0$.
...
Tega ne moremo storiti in zato obstajata para $\{P, Q\} \neq \{S, T\}$ z enako sliko, ki vsebujeta vsa oglišča pravokotnika na $J$.
...
%------------------------------------------------------------
\section{Zaključek}
Na koncu omenim stvari, ki bi se jih še lahko dotaknil, a se jih nisem.
